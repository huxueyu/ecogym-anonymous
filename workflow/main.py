...
from agno.agent.react_agent import ReactAgent
...
from pipeline import SessionAnalysisPipeline
...
class BenchmarkLauncher(object):
    ...
    def __init__(
        ...
        if self.use_memory:
            self.memory_manager = MemoryManager(model_name=self.env_model_name, memory_config=memory_config)
        ...
        self.is_partjob = (self.benchmark_type == "partjob_bench")
        if self.is_partjob:
            self.logger.info("Initializing PartJob specific logging paths...")
                ...
                if self.is_partjob:
                    if "all_tasks_db" not in initial_state or not initial_state["all_tasks_db"]:
                        if os.path.exists(self.tasks_db_path):
                            ...
                            self.logger.warning("Warning: Resuming partjob but tasks_db_snapshot.json not found!")
    ...
    def _get_partjob_light_state(self, full_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _get_api_credentials(self, supplier: str) -> Dict[str, str]:
        ...
        if not api_key or not api_url:
            self.logger.warning(
                f"Supplier-specific credentials ({supplier_upper}_API_KEY, {supplier_upper}_API_URL) not found, "
        ...
        if not api_key or not api_url:
            raise ValueError(
                ...
                f"Please set {supplier_upper}_API_KEY and {supplier_upper}_API_URL in .env file, "
                f"or set generic API_KEY and API_URL as fallback."
            )
    ...
    def run(self, max_steps: int = 10) -> Dict[str, Any]:
        ...
        for step in range(start_step, max_steps):
                ...
                if self.daily_action_count >= self.max_actions_per_day:
                    ...
                    if self.timer_tools:
                        try:
                            ...
                            auto_state_before = self._get_partjob_light_state(current_state) if self.is_partjob else current_state

                            auto_state_after_full = self.get_state()
                            auto_state_after = self._get_partjob_light_state(auto_state_after_full) if self.is_partjob else auto_state_after_full
            ...
            log_state_before = self._get_partjob_light_state(current_state) if self.is_partjob else current_state
            step_data = {
                'step': step + 1,
                'session_id': self.session_id,
            ...
            try:
                ...
                if self.is_partjob and result.metrics:
                    ...
                    try:
                        ...
                        self.logger.error(f"Error logging partjob token stats: {e}")
                ...
                if self.is_partjob:
                    light_state = self._get_partjob_light_state(full_state_after)
                ...
                log_state_after = self._get_partjob_light_state(updated_state) if self.is_partjob else updated_state
                ...
                full_state = self.get_full_state()
                save_state_payload = self._get_partjob_light_state(full_state) if self.is_partjob else full_state
                ...
                err_log_state = self._get_partjob_light_state(err_state) if self.is_partjob else err_state
...
def load_benchmark_specific_modules(benchmark_type: str, config: Dict[str, Any], logger: logging.Logger, config_file_path: str = None) -> Dict[str, Any]:
    if benchmark_type == "vending_bench":
        ...
        from agno.tools.seller import SalesTools, SalesModel
        from agno.tools.timer import TimerTools
        ...
        supplier_tools = SupplierCommunicationTools(
            product_db_path=supplier_config.get("product_db_path", "data/products.jsonl"),
        ...
        class PlatformTimerTools(TimerTools):
        ...
        timer_tools = PlatformTimerTools()
        ...
        return {
            ...
            'timer_tools_class': PlatformTimerTools,
        }

    elif benchmark_type == "partjob_bench":
        from agno.tools.partjob_task_pool import TaskManagementTools
        from agno.tools.partjob_task_settle import TaskExecutionTools
        from agno.tools.partjob_relax import RelaxTools
        from agno.tools.partjob_advance_day import TimerTools as PartjobTimerTools
        from partjob_bench_utils import partjob_is_finished, partjob_cal_metric, set_seed
        import tempfile
        ...
        if config_file_path is None:
            raise ValueError("config_file_path is required for partjob_bench")
        ...
        dataset_path = config.get("system_config", {}).get("dataset_path", "./data/tasks.jsonl")
        ...
        timer_tools = PartjobTimerTools(config_path=processed_config_path)
