...
try:
    import chromadb
    from chromadb.api.client import Client as ClientAPI
...
class ChromaDb(VectorDb):
    ...
    async def get_nearest_neighbors(
        ...
        """Get k nearest neighbors for a text using vector similarity."""
        query_embedding = await self.embedder.aget_embedding(text)
        ...
        where_filter = self._convert_filters(where) if where else None
    ...
    def batch_upsert(
        ...
        """Batch upsert documents with specified batch size."""
        for i in range(0, len(documents), batch_size):
            batch = documents[i:i + batch_size]
            self.upsert(
                content_hash=f"batch_{i//batch_size}",
                documents=batch
            )
    ...
    def _update_embeddings(
        ...
        """Update embeddings for documents in collection."""
